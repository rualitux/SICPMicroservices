    ...
    public static class PrepDb
    {
        public static void PrepPopulation(IApplicationBuilder app)
        {
            ...
            using (var serviceScope = app.ApplicationServices.CreateScope())
            {
                ...
                var grpcClient = serviceScope.ServiceProvider.GetService<IEnumeradoDataClient>();
                var enumerados = grpcClient.ReturnAlLEnumerados();
                SeedData(serviceScope.ServiceProvider.GetService<IBienPatrimonialRepository>(), enumerados);
                //SeedData(serviceScope.ServiceProvider.GetService<AppDbContext>());

            }
        }
        private static void SeedData(IBienPatrimonialRepository repository, IEnumerable<Enumerado> enumerados)
        {
            Console.WriteLine("Seedeando Enumerados...");
            foreach (var enumerado in enumerados)
            {
                if (!repository.EnumeradoExternoExiste(enumerado.ExternalId))
                {
                    repository.CreateEnumerado(enumerado);
                }
                repository.Save();
            }
        }
